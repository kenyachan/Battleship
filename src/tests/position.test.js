const { Position, Coordinates } = require('../position');

describe('Coordinates Testing', () => {
	let coordinates;
	let randomX;
	let randomY;
...
describe('Position Testing', () => {
	test('Position will have same number of coordinates as given length', () => {
		let position = new Position(new Coordinates(0,0), 5, 'horizontal');

		expect(position.getCoordinates()).toHaveLength(5);
	});
});
